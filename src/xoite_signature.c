...
er_t xoite_signature_s_parse( xoite_signature_s* o, bcore_source* source )
{
    ...
    BLM_TRY( xoite_group_s_parse_name( o->group, name_buf, source ) );
    ...
    if( !bcore_source_a_parse_bl_fa( source, " #=?'*'" ) )
    {
        if( bcore_source_a_parse_bl_fa( source, " #?':'" ) )
        {
            ...
            XOITE_BLM_SOURCE_PARSE_FA( source, " #name", name_candidate );
            if( name_buf->size == 0 )
            {
        ...
        else if( bcore_source_a_parse_bl_fa( source, " #?'@'" ) )
        {
            ...
            {
                XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Misplaced '@'." );
            }
        }
        ...
        {
            XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Syntax error." );
        }
    }
    ...
    {
        // get return type
        if( name_buf->size == 0 ) XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Return type missing." );
        st_s_push_fa( &o->ret_type, "#<sc_t>", name_buf->sc );
        if( st_s_equal_sc( name_buf, "const" ) )
        {
            XOITE_BLM_SOURCE_PARSE_FA( source, " #name", name_buf );
            if( name_buf->size == 0 ) XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Return type missing." );
            st_s_push_fa( &o->ret_type, " #<sc_t>", name_buf->sc );
        }
    ...
    if( o->name.size == 0 )
    {
        ...
        {
            XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Name missing." );
        }
    }
    ...
    if( !predefined )
    {
        ...
        else     XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "'plain', mutable' or 'const' expected." );

        BLM_TRY( xoite_args_s_parse( &o->args, source ) );
        XOITE_BLM_SOURCE_PARSE_FA( source, " ) " );
    }
    else if( bcore_source_a_parse_bl_fa( source, " #?'('" ) )
    {
        BLM_TRY( xoite_args_s_append( &o->args, source ) );
        XOITE_BLM_SOURCE_PARSE_FA( source, " ) " );
    }
