...
tp_t xoite_feature_s_get_hash( const xoite_feature_s* o )
{
    ...
    if( o->default_body )
    {
        hash = bcore_tp_fold_tp( hash, xoite_body_s_get_hash( o->default_body ) );
    }
    ...
    hash = bcore_tp_fold_tp( hash, xoite_args_s_get_hash( &o->args ) );
...
er_t xoite_feature_s_parse( xoite_feature_s* o, bcore_source* source )
{
    ...
    if( bcore_source_a_parse_bl_fa( source, " #?|'|" ) )
    {
        ...
        XOITE_BLM_SOURCE_PARSE_FA(  source, " #until'''", flags );
        for( sz_t i = 0; i < flags->size; i++ )
        {
            switch( flags->data[ i ] )
            {
                ...
                default: XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Feature: Flag '#<char>' not handled. Choose from 'ptar'.", flags->data[ i ] ); break;
            }
        }
    ...
    {
        XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Feature: '<flags>' expected. Example: 'ptar'" );
    }
    ...
    xoite_args_s_copy( &o->args, &signature->args );

    if( !signature->arg_o )
    {
        XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Feature: The first argument must be 'mutable' or 'const'." );
    }
    ...
    if( bcore_source_a_parse_bl_fa( source, " #?'=' " ) )
    {
        if( bcore_source_a_parse_bl_fa( source, " #=?'{' " ) )
        {
            if( o->strict ) XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Feature is 'strict'. Default function would have no effect." );
            o->default_body = xoite_body_s_create();
            o->default_body->group = o->group;
            BLM_TRY( xoite_body_s_parse_code( o->default_body, NULL, source ) );
            st_s_copy_fa( &o->default_name, "#<sc_t>__", o->name.sc );
        }
        else
        {
            if( o->strict )  XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Feature is 'strict'. Default function would have no effect." );
            XOITE_BLM_SOURCE_PARSE_FA( source, " #name ", &o->default_name );
            if( o->default_name.size == 0 ) XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Feature: Default function name expected." );
            if( st_s_equal_st( &o->default_name, &o->name ) )
            {
                XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Feature: Default function name must differ from feature name." );
            }
        }
    }
...
er_t xoite_feature_s_expand_indef_typedef( const xoite_feature_s* o, sz_t indent, bcore_sink* sink )
{
    ...
    BLM_TRY( xoite_args_s_expand( &o->args, false, sink ) );
    bcore_sink_a_push_fa( sink, " );" );
    BLM_RETURNV( er_t, 0 );
}
...
er_t xoite_feature_s_expand_spect_definition( const xoite_feature_s* o, sz_t indent, bcore_sink* sink )
{
...
er_t xoite_feature_s_expand_indef_declaration( const xoite_feature_s* o, sz_t indent, bcore_sink* sink )
{
    ...
    if( o->flag_p )
    {
        ...
        BLM_TRY( xoite_args_s_expand_name( &o->args, false, sink ) );
        bcore_sink_a_push_fa( sink, " ); }" );
    }
    ...
    if( o->default_name.size > 0 )
    {
        if( o->default_body && o->default_body->go_inline )
        {
            ...
            BLM_TRY( xoite_args_s_expand( &o->args, false, sink ) );
            bcore_sink_a_push_fa( sink, " )" );
            BLM_TRY( xoite_body_s_expand( o->default_body, indent, sink ) );
        }
        else
        {
            // ret_t feature_default( feature* o, arg_t arg1 );
