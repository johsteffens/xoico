...
static er_t take_block( xoico_cbeth_s* o, sz_t level, bcore_source* source, bcore_sink* sink );
...
static er_t take_line_comment( xoico_cbeth_s* o, bcore_source* source, bcore_sink* sink )
{
    BLM_INIT();
...
static er_t take_block_comment( xoico_cbeth_s* o, bcore_source* source, bcore_sink* sink )
{
    BLM_INIT();
    BLM_TRY( bcore_source_a_parse_em_fa( source, "/*" ) );
    bcore_sink_a_push_fa( sink, "/*" );
...
static er_t take_whitespace( xoico_cbeth_s* o, bcore_source* source, bcore_sink* sink )
{
    BLM_INIT();
    while( !bcore_source_a_eos( source ) )
    {
...
static er_t take_string( xoico_cbeth_s* o, bcore_source* source, bcore_sink* sink )
{
    BLM_INIT();
    BLM_TRY( bcore_source_a_parse_em_fa( source, "\"" ) );
...
static er_t take_preprocessor( xoico_cbeth_s* o, bcore_source* source, bcore_sink* sink )
{
    BLM_INIT();
    BLM_TRY( bcore_source_a_parse_em_fa( source, "##" ) );
    bcore_sink_a_push_fa( sink, "##" );
...
static er_t take_statement_name( xoico_cbeth_s* o, sz_t level, bcore_source* source, bcore_sink* sink )
{
    BLM_INIT();
    ...
    if( xoico_compiler_s_is_type( o->compiler, btypeof( st_name->sc ) ) )
    {
        ...
        if( bcore_source_a_parse_bl_fa( source, "#?'*'" ) )
        {
            ...
            {
                xoico_cbeth_tn_stack_s_push_sc( &o->stack, st_type->sc, st_name->sc, level );
            }
        }
    }
    else
    {
        ...
        if( bcore_source_a_parse_bl_fa( source, "#?'->'" ) )
        {
            ...
            if( st_fname->size > 0 )
            {
                ...
                {
                    bcore_sink_a_push_sc( sink_buf, "(" );
                    sc_t  sc_type = xoico_cbeth_tn_stack_s_get_type_sc( &o->stack, st_name->sc );
...
static er_t take_bracket( xoico_cbeth_s* o, sz_t level, bcore_source* source, bcore_sink* sink )
{
    BLM_INIT();
...
/** A statement in cbeth differs semantically from c-statement
 *  It is used here for code-segmentation.
 */
static er_t take_statement( xoico_cbeth_s* o, sz_t level, bcore_source* source, bcore_sink* sink )
{
    BLM_INIT();
    while( !bcore_source_a_eos( source ) )
    {
...
static er_t take_block_body( xoico_cbeth_s* o, sz_t level, bcore_source* source, bcore_sink* sink )
{
    BLM_INIT();
    while( !bcore_source_a_eos( source ) )
    {
        ...
        else if( c == '}' )
        {
            ...
            xoico_cbeth_tn_stack_s_pop( &o->stack, level );
            break;
        }
        else if( c == '(' )
        {
...
{
    xoico_cbeth_tn_stack_s_init_from_args( &o->stack, o->obj_type, "o", o->args );
...
er_t xoico_cbeth_s_translate( const xoico_cbeth_s* o, const xoico_body_s* body, const xoico_signature_s* signature, bcore_sink* sink )
{
    ...
    if( xoico_cbeth_s_take_block_body( aleph, source, sink ) )
    {
        er_t id = 0;
