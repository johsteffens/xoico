...
sc_t xoite_stamp_s_get_rel_name_sc( const xoite_stamp_s* o )
{
...
tp_t xoite_stamp_s_get_hash( const xoite_stamp_s* o )
{
    ...
    {
        hash = bcore_tp_fold_tp( hash, xoite_func_s_get_hash( o->funcs.data[ i ] ) );
    }
    return hash;
}
    ...
    for( sz_t i = 0; i < string->size; i++ )
    {
        ...
        switch( c )
        {
            case '@':
            {
                ...
                {
                    XOITE_BLM_SOURCE_POINT_PARSE_ERR_FA
                    (
                        &o->source_point,
            ...
            case '$':
            {
                ...
                switch( c )
                {
                    case 'R':
                    {
                        sc_t rel_name = xoite_stamp_s_get_rel_name_sc( o );
...
er_t xoite_stamp_s_extend( xoite_stamp_s* o, bcore_source* source, bl_t verbatim )
{
    ...
    while( !bcore_source_a_eos( source ) && !bcore_source_a_parse_bl_fa( source, " #?'}'" ) )
    {
        if( bcore_source_a_parse_bl_fa( source, " #?w'func'" ) )
        {
            ...
            if( idx >= 0 )
            {
                xoite_func_s* prex_func = o->funcs.data[ idx ];
                if( prex_func->overloadable )
                {
                    BLM_TRY( xoite_funcs_s_replace_fork( &o->funcs, idx, func ) );
                    st_s_replace_sc_sc( o->self_source, prex_func->decl.sc, "" );
                    if( register_func ) st_s_push_st( o->self_source, &func->decl );
                }
                else
                {
                    XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Function '#<sc_t>' has already been defined and is not overloadable.", func->name.sc );
                }
            }
            else
            {
    ...
    if( !verbatim )
    {
        for( xoite_group_s* fgroup = o->group; fgroup != NULL; fgroup = fgroup->group )
        {
            ...
            {
                xoite_func_s* func = fgroup->funcs.data[ i ];
                if( !xoite_funcs_s_exists( &o->funcs, func->type ) )
                {
                    if( xoite_func_s_registerable( func ) ) st_s_push_st( o->self_source, &func->decl );
...
er_t xoite_stamp_s_parse( xoite_stamp_s* o, xoite_group_s* group, bcore_source* source )
{
    ...
    BLM_TRY( xoite_group_s_parse_name( group, stamp_name, source ) );
    ...
    {
        xoite_stamp_s_copy( o, group->extending );
    }
    else
    {
        st_s_push_fa( o->self_source, "@ =" );
        ...
        else
        {
            ...
            if( trait_name->size == 0 ) XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Trait name expected." );
        }
...
er_t xoite_stamp_s_finalize( xoite_stamp_s* o )
{
    ...
    BLM_TRY( xoite_stamp_s_resolve_chars( o, o->self_source ) );
    for( sz_t i = 0; i < o->funcs.size; i++ )
    {
...
er_t xoite_stamp_s_arg_s_expand( const xoite_stamp_s* o, const xoite_arg_s* arg, bcore_sink* sink )
{
    BLM_INIT();
    st_s* arg_type = BLM_CLONE( st_s, &arg->type );
    BLM_TRY( xoite_stamp_s_resolve_chars( o, arg_type ) );
...
er_t xoite_stamp_s_args_s_expand( const xoite_stamp_s* o, const xoite_args_s* args, bl_t first, bcore_sink* sink )
{
    BLM_INIT();
    for( sz_t i = 0; i < args->size; i++ )
    {
        ...
        first = false;
        BLM_TRY( xoite_stamp_s_arg_s_expand( o, &args->data[ i ], sink ) );
    }
    BLM_RETURNV( er_t, 0 );
}
...
er_t xoite_stamp_s_expand_declaration( const xoite_stamp_s* o, sz_t indent, bcore_sink* sink )
{
    ...
    for( sz_t i = 0; i < o->funcs.size; i++ )
    {
        ...
        if( xoite_compiler_s_item_exists( xoite_group_s_get_compiler( o->group ), func->type ) )
        {
            ...
            if( *(aware_t*)item == TYPEOF_xoite_feature_s )
            {
                ...
                BLM_TRY( xoite_args_s_expand( &feature->args, false, sink ) );
                bcore_sink_a_push_fa( sink, " )" );

                if( go_inline )
                {
                    xoite_body_s_expand( func->body, indent, sink );
                }
                else
                {
                    bcore_sink_a_push_fa( sink, ";" );
            ...
            else if( *(aware_t*)item == TYPEOF_xoite_signature_s )
            {
                ...
                BLM_TRY( xoite_stamp_s_resolve_chars( o, ret_type ) );
                ...
                if( signature->arg_o )
                {
                    ...
                    bcore_sink_a_push_fa( sink, "#<sc_t>* o", o->name.sc );
                    BLM_TRY( xoite_stamp_s_args_s_expand( o, &signature->args, false, sink ) );
                }
                else
                {
                    if( signature->args.size > 0 )
                    {
                        BLM_TRY( xoite_stamp_s_args_s_expand( o, &signature->args, true, sink ) );
                    }
                    else
                    {
                ...
                if( go_inline )
                {
                    BLM_TRY( xoite_body_s_expand( func->body, indent, sink ) );
                }
                else
                {
                    bcore_sink_a_push_fa( sink, ";" );
            ...
            {
                XOITE_BLM_SOURCE_POINT_PARSE_ERR_FA( &func->source_point, "Stamp #<sc_t>: Could not resolve function #<sc_t> #<sc_t>", o->name.sc, ifnameof( func->type ), func->name.sc );
            }
        }
        else
        {
    ...
    if( self->trait == TYPEOF_bcore_array )
    {
        ...
        if( !array_item )
        {
            XOITE_BLM_SOURCE_POINT_PARSE_ERR_FA( &o->source_point, "Expanding object #<sc_t>: Object is of trait array but contains no array.", o->name.sc );
        }
...
er_t xoite_stamp_s_expand_init1( const xoite_stamp_s* o, sz_t indent, bcore_sink* sink )
{
    ...
    for( sz_t i = 0; i < o->funcs.size; i++ )
    {
        ...
        if( xoite_compiler_s_item_exists( xoite_group_s_get_compiler( o->group ), func->type ) )
        {
            ...
            {
                if( xoite_func_s_registerable( func ) )
                {
                    const xoite_signature_s* signature = ( xoite_signature_s* )item;
