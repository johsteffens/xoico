...
tp_t xoite_target_s_get_hash( const xoite_target_s* o )
{
    ...
    BFOR_EACH( i, &o->dependencies )
    {
        ...
        xoite_target_s* dep_target = o->compiler->data[ idx ];
        hash = bcore_tp_fold_tp( hash, xoite_target_s_get_hash( dep_target ) );
    }
...
{
    BLM_INIT();
    for( sz_t i = 0; i < o->size; i++ ) BLM_TRY( xoite_source_s_finalize( o->data[ i ] ) );
    BLM_RETURNV( er_t, 0 );
}
...
bl_t xoite_target_s_is_cyclic_recursive( xoite_target_s* o )
{
    ...
    {
        sz_t idx = o->dependencies.data[ i ];
        if( xoite_target_s_is_cyclic_recursive( o->compiler->data[ idx ] ) ) return true;
    }
    return false;
}
...
void xoite_compiler_s_clear_flags( xoite_compiler_s* o )
{
...
{
    xoite_compiler_s_clear_flags( o->compiler );
    bl_t cyclic = xoite_target_s_is_cyclic_recursive( o );
    xoite_compiler_s_clear_flags( o->compiler );
    return cyclic;
}
...
er_t xoite_target_s_expand_heading( const xoite_target_s* o, sz_t indent, bcore_sink* sink )
{
    BLM_INIT();
    ...
    for( sz_t i = 0; i < o->size; i++ )
    {
        xoite_source_s* source = o->data[ i ];
...
er_t xoite_target_s_expand_h( const xoite_target_s* o, sz_t indent, bcore_sink* sink )
{
    BLM_INIT();
    xoite_target_s_expand_heading( o, indent, sink );
    ...
    for( sz_t i = 0; i < o->size; i++ ) BLM_TRY( xoite_source_s_expand_declaration( o->data[ i ], indent, sink ) );
...
er_t xoite_target_s_expand_init1( const xoite_target_s* o, sz_t indent, bcore_sink* sink )
{
    return 0;
}
...
er_t xoite_target_s_expand_c( const xoite_target_s* o, sz_t indent, bcore_sink* sink )
{
    BLM_INIT();
    BLM_TRY( xoite_target_s_expand_heading( o, indent, sink ) );
    ...
    /// definition section
    bcore_sink_a_push_fa( sink, "\n" );
    for( sz_t i = 0; i < o->size; i++ ) BLM_TRY( xoite_source_s_expand_definition( o->data[ i ], indent, sink ) );
    ...
    BFOR_EACH( i, &o->dependencies )
    {
        const xoite_target_s* target = o->compiler->data[ o->dependencies.data[ i ] ];
    ...
    bcore_sink_a_push_fa( sink, "#rn{ }        {\n", indent );
    BLM_TRY( xoite_target_s_expand_init1( o, indent + 12, sink ) );
    for( sz_t i = 0; i < o->size; i++ ) BLM_TRY( xoite_source_s_expand_init1( o->data[ i ], indent + 12, sink ) );
    bcore_sink_a_push_fa( sink, "#rn{ }        }\n", indent );
...
er_t xoite_target_s_expand_phase1( xoite_target_s* o, bl_t* p_modified )
{
    ...
    if( xoite_target_s_to_be_modified( o ) )
    {
...
er_t xoite_target_s_expand_phase2( xoite_target_s* o, bl_t* p_modified )
{
    ...
    BLM_TRY( xoite_compiler_s_check_overwrite( o->compiler, file_h->sc ) );
    BLM_TRY( xoite_compiler_s_check_overwrite( o->compiler, file_c->sc ) );
