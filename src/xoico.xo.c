...
er_t xoico_typespec_s_parse( xoico_typespec_s* o, const xoico_host* host, bcore_source* source )
{
    ...
    if( bcore_source_a_parse_bl(source," #?w'const'" ) ) BLM_TRY(bcore_source_a_parse_error_fa(source,"Abbreviate 'const' to 'c'." ))
    
    if(      bcore_source_a_parse_bl(source," #?w'c'" )                                           ) access_class = TYPEOF_const;
...
er_t xoico_stamp_s_parse_extend( xoico_stamp_s* o, bcore_source* source )
{
    ...
    while( !bcore_source_a_eos(source) && !bcore_source_a_parse_bl(source," #?'}'" ) )
    {
        ...
        {BLM_INIT_LEVEL(2);
            ...
            er_t er = bcore_self_item_s_parse_src( item, sr_awc( ((bcore_source_string_s*)BLM_LEVEL_T_PUSH(0,bcore_source_string_s,bcore_source_string_s_create_from_string(buf ))) ), self, false );
...
er_t xoico_che_s_try_take_typespec( xoico_che_s* o, bcore_source* source, xoico_typespec_s* typespec, bl_t require_tractable_type, bl_t* success )
{
    ...
    if( literal_const_used ) BLM_TRY(bcore_source_a_parse_error_fa(source,"Abbreviate 'const' to 'c'." ))
...
// XOILA_OUT_SIGNATURE 0x470DBF3C9A128CBAull
