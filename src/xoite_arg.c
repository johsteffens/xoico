...
er_t xoite_arg_s_parse( xoite_arg_s* o, bcore_source* source )
{
    ...
    {
        xoite_group_s_parse_name_recursive( o->group, s, source );
    }
    ...
    else
    {
        ...
        if( s->size == 0 ) XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Argument: Type expected." );
    }
    ...
    XOITE_BLM_SOURCE_PARSE_FA( source, "#name ", s );
    if( s->size == 0 ) XOITE_BLM_SOURCE_PARSE_ERR_FA( source, "Argument: Name expected." );
